...
class Admin extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        $guru = $this->db->get_where('admin', ['email_guru' => $email_guru])->row_array();
        if ($guru) {
            if ($guru['is_active'] == 1) {
    ...
    public function verify()
    {
        ...
        $user = $this->db->get_where('admin', ['email_guru' => $email])->row_array();
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->db->delete('admin', ['email_guru' => $email]);
                    $this->db->delete('user_token', ['email' => $email]);
    ...
    public function forgotPassword()
    {
        $this->form_validation->set_rules('email_guru', 'email', 'required|trim|valid_email');
        if ($this->form_validation->run() == false) {
            $data['judul'] = '[admin] Halaman Lupa Kata Sandi';
            $this->load->view('admin/templates/auth_header', $data);
            $this->load->view('admin/auth/forgotPassword', $data);
            $this->load->view('admin/templates/auth_footer');
        } else {
            $email = $this->input->post('email_guru');
            $user = $this->db->get_where('admin', ['email_guru' => $email, 'is_active' => 1])->row_array();
            if ($user) {
    ...
    public function changePassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['judul'] = '[admin] Halaman Reset Kata Sandi';
            $this->load->view('admin/templates/auth_header', $data);
    ...
    private function _sendEmail($token, $type)
    {
        ...
        if ($type == "verify") {
            ...
            $this->email->message('Klik tautan ini untuk verifikasi akun anda : ' . base_url() . 'admin/verify?email=' . $this->input->post('email_guru') . '&token=' . urlencode($token) . '');
        } else if ($type == "forgot") {
            $this->email->subject('Reset kata sandi');
            $this->email->message('Klik tautan ini untuk reset kata sandi anda : ' . base_url() . 'admin/reset?email=' . $this->input->post('email_guru') . '&token=' . urlencode($token) . '');
        }
        if ($this->email->send()) {
            return true;
        } else {
